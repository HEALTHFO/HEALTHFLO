...
function coreServices(h) {
  ...
  return [
    { icon: '🩺', title: 'OPD', meta: 'Mon–Sat • 9 AM – 7 PM', action: window.t('hosp_book_opd'), on: () => toast('OPD booking started') },
    { icon: '🛏️', title: 'IPD', meta: `${h.beds || 120}+ ${window.t('hosp_beds')} • Private & Shared`, action: window.t('hosp_plan_adm'), on: () => toast('IPD admission team alerted') },
    { icon: '📱', title: 'Teleconsult', meta: 'Video / Audio', action: window.t('hosp_start_call'), on: () => toast('Teleconsult link shared') },
    { icon: '🚑', title: 'Emergency', meta: `${distance} km • 24×7 desk`, action: window.t('hosp_call_er'), on: () => window.open('tel:108', '_self') },
  ];
}
...
function createCard(h, medsFlat) {
  ...
  card.innerHTML = `
    <figure class="card-visual">
      ...
      <div class="card-visual-badges">
        <div class="row">
          ...
          <span class="pill badge">${h.isCashless ? window.t('hosp_cashless_desk') : window.t('hosp_selfpay')}</span>
        </div>
        <div class="row">
          <span class="pill badge">${distance} km live</span>
          <span class="pill badge">${wait} ${window.t('hosp_min_wait')}</span>
        </div>
      </div>
    </figure>
    <div class="card-top">
      ...
      <div class="card-info-grid">
        <div class="card-info"><strong>${h.beds || 140}+ ${window.t('hosp_beds')}</strong><span>${window.t('hosp_live_bed')}</span></div>
        <div class="card-info"><strong>${wait} min</strong><span>${window.t('hosp_opd_wait')}</span></div>
        <div class="card-info"><strong>${occupancy}%</strong><span>${window.t('hosp_occupancy')}</span></div>
      </div>
    <div class="card-inline">
      <span class="chip-flat">${distance} km • Smart ETA</span>
      <span class="chip-flat">${(h.specialties || []).length || 3} specialties</span>
      <span class="chip-flat">${h.isCashless ? window.t('hosp_cashless') : window.t('hosp_selfpay')}</span>
      <span class="chip-flat">AI concierge</span>
    </div>
  </div>
    <div class="card-explorer">
      <div class="card-tabs" role="tablist">
        <button class="card-tab active" data-pane="services" type="button">${window.t('hosp_tab_services')}</button>
        <button class="card-tab" data-pane="packages" type="button">${window.t('hosp_tab_packages')}</button>
        <button class="card-tab" data-pane="doctors" type="button">${window.t('hosp_tab_doctors')}</button>
        <button class="card-tab" data-pane="pharmacy" type="button">${window.t('hosp_tab_pharmacy')}</button>
        <button class="card-tab" data-pane="labs" type="button">${window.t('hosp_tab_labs')}</button>
      </div>
    ...
    <div class="card-actions">
      <button class="btn ghost" type="button">${window.t('hosp_btn_route')}</button>
      <button class="btn primary" type="button">${window.t('hosp_btn_view')}</button>
      <button class="btn emergency" type="button">${window.t('hosp_btn_emergency')}</button>
    </div>
  `;
...
function renderGlance(h, distance) {
  ...
  const cards = [
    ...
    { title: window.t('hosp_cashless'), value: h.isCashless ? 'Available' : window.t('hosp_selfpay'), meta: window.t('hosp_insurance_desk') },
    { title: 'Distance', value: `${distance} ${window.t('hosp_distance')}`, meta: window.t('hosp_live_eta') },
    { title: 'OPD load', value: `${wait} min`, meta: window.t('hosp_opd_wait') },
    { title: 'Signals', value: `${crowd}% ${window.t('hosp_occupancy_val')}`, meta: window.t('hosp_occupancy') },
